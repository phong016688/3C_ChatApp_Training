...
class AdapterFriendList(context: Context?, listFriend: ArrayList<User>) :
    ...
    inner class FriendHolder(itemView: View) : RecyclerView.ViewHolder(itemView),
        ...
        override fun onClick(p0: View?) {
            mItemClickListener?.onClick(p0, adapterPosition)
        }

        fun onBind(position: Int) {
            itemView.itemAvatarImageView.loadImageUser(mListFriend[position].avatar)
            itemView.itemUserNameText.text = mListFriend[position].userName
        }
    }
}
